...
public:
    ...
    QVector<QString> getUserProfile(int id_, const QString& access_token, const QString& service){
...
class stubRegistrationManager : RegistrationManager{
 public:
     User registerUser(const QString& login, const QString& email, const QString& password){
     ...
     bool validateUserDetails(const User& user){
        ...
        if (user.getLogin().isEmpty() || user.getEmail().isEmpty() || user.getPassword().isEmpty()) {
            return false;
        }
...
TEST(RegistrationManager, validateUserDetails){
     stubRegistrationManager sRG;
     bool state =  sRG.validateUserDetails(sRG.registerUser("login", "@email@@", "password12345"));
...
TEST(RegistrationManager, validateUserDetails_f){
     stubRegistrationManager sRG;
     bool state =  sRG.validateUserDetails(sRG.registerUser("login", "email", "password"));
