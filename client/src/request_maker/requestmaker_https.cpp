...
#include"request_https.hpp"
    ...
    void RequestMaker::Get()
    {
        ...
        client(std::make_shared<IClientHttps>(ioc, ctx));
        http::request<http::string_body> request{http::verb::get, path, 11};
        request.set(http::field::host, host_);
        request.set(http::field::user_agent, BOOST_BEAST_VERSION_STRING);
        std::thread thr(this->CallBack, request);
    }; 
    ...
    {
        client_->Run(host_, port_, target_, request);
        auto messsage = client_->GetResponse();
